...
**Four of the five are machine captured, not written by hand.**
`envguard/auditor.py` accepts a `trace` argument that records every step of a real
run, and `capture.py` renders those recordings into `01`, `02`, `03` and `05`.
...
| [`04-coding-agent.md`](04-coding-agent.md) | **The coding agent that built this repository.** Nine checkpoints where a tool response or a committed artefact contradicted the agent and changed the design: the harness bypass, a wrong answer key, a false confirmation, a fabricated measurement, a conclusion its own later data reversed, defects found by external reviewers, documentation that described a system the code no longer was, a review round that attacked the experiment rather than the code and took the headline number apart, and four simultaneous reviews that found four sentences the repository's own files refute. |
